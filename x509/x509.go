...
func checkSignature(algo SignatureAlgorithm, signed, signature []byte, publicKey crypto.PublicKey) (err error) {
	...
	case *ecdsa.PublicKey:
		...
		switch pub.Curve {
		case sm2.GetSm2P256V1():
			sm2pub := sm2.ToSm2Publickey(pub)
			//sm2pub := &sm2.PublicKey{
			//	Curve: pub.Curve,
			//	X:     pub.X,
			//	Y:     pub.Y,
			//}
			if !sm2.VerifyByRS(sm2pub, nil, signed, ecdsaSig.R, ecdsaSig.S) {
...
func (c *Certificate) GetTemplateHash() []byte {
	template := Certificate{
		...
		Subject: pkix.Name{
			...
			ExtraNames:         c.Subject.ExtraNames,
		},
		NotBefore:          c.NotBefore,
		NotAfter:           c.NotAfter,
		SignatureAlgorithm: c.SignatureAlgorithm,
	}
	byteTemplate, _ := asn1.Marshal(template)

	return HashSumByte(byteTemplate)
}
...
// RsaSignVer 中心化CA验证签名
func (c *Certificate) RsaSignVer(rootPublicKey string) error {
	//pem解码
	block, _ := pem.Decode([]byte(rootPublicKey))
	//x509解码
	publicKeyInterface, err := x509.ParsePKIXPublicKey(block.Bytes) //PKCS #8
	if err != nil {
		return err
	}
	//类型断言
	...
	hash := sha256.New()
	hash.Write(c.RawTBSCertificate)
	hashText := hash.Sum(nil)
	return rsa.VerifyPKCS1v15(publicKey, crypto.SHA256, hashText, c.Signature)
}

func (c *Certificate) SmSignVer(rootPublicKey string) error {
	//pem解码
	block, _ := pem.Decode([]byte(rootPublicKey))
	// sm解码
	publicKey, err := ParseSm2PublicKey(block.Bytes)
	if err != nil {
		return err
	}
	verify := sm2.VerifyByAsn(publicKey, nil, c.RawTBSCertificate, c.Signature)
	if verify {
		return nil
	} else {
