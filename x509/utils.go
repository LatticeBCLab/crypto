...
func CreateCertificateEcdsa(template, parent *Certificate, publicKey *ecdsa.PublicKey, signer crypto.Signer) ([]byte, error) {
	if template.SerialNumber == nil {
		return nil, errors.New("x509: no SerialNumber given")
	}
...
func CreateNodeCertificate(template *Certificate, pub *sm2.PublicKey, digest []byte, sigs []byte) ([]byte, error) {
	_, signatureAlgorithm, err := signingParamsForPublicKey(pub, template.SignatureAlgorithm)
	if err != nil {
		return nil, err
	}
	c := tbsCertificate{}
	_, err = asn1.Unmarshal(digest, &c)
	c.Raw = digest
	return asn1.Marshal(certificate{
		nil,
		c,
		signatureAlgorithm,
		asn1.BitString{Bytes: sigs, BitLength: len(sigs) * 8},
	})
}
func CreateNodeCertificateEcdsa(pub *ecdsa.PublicKey, digest []byte, sigs []byte, requestedSigAlgo SignatureAlgorithm) ([]byte, error) {
	c := tbsCertificate{}
	_, err := asn1.Unmarshal(digest, &c)
	c.Raw = digest
	_, signatureAlgorithm, err := signingParamsForPublicKey(pub, requestedSigAlgo)
	if err != nil {
		return nil, err
	}
	return asn1.Marshal(certificate{
...
func GetTbsCertificateDigest(template *Certificate, publicKey *sm2.PublicKey) ([]byte, error) {
	if template.SerialNumber == nil {
		return nil, errors.New("x509: no SerialNumber given")
	}
	...
	c := tbsCertificate{
		...
		SerialNumber:       template.SerialNumber,
		SignatureAlgorithm: signatureAlgorithm,
		Issuer:             asn1.RawValue{FullBytes: asn1Subject},
		Validity:           validity{template.NotBefore.UTC(), template.NotAfter.UTC()},
		Subject:            asn1.RawValue{FullBytes: asn1Subject},
		PublicKey:          publicKeyInfo{nil, publicKeyAlgorithm, encodedPublicKey},
		Extensions:         extensions,
...
func GetTbsCertificateDigestEcdsa(template *Certificate, publicKey *ecdsa.PublicKey) ([]byte, error) {
	if template.SerialNumber == nil {
		return nil, errors.New("x509: no SerialNumber given")
	}
	_, signatureAlgorithm, err := signingParamsForPublicKey(publicKey, template.SignatureAlgorithm)
...
func CreateCertificateToPem(template, parent *Certificate, pubKey *ecdsa.PublicKey, signer crypto.Signer) ([]byte, error) {
	der, err := CreateCertificateEcdsa(template, parent, pubKey, signer)
	if err != nil {
		return nil, err
	}
	block := &pem.Block{
