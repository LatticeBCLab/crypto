...
import (
	...
	"github.com/LatticeBCLab/crypto/gm/util"
	"hash"
	"io"
	"math/big"
)
...
func (pri *PrivateKey) Sign(rand io.Reader, digest []byte, opts crypto.SignerOpts) (signature []byte, err error) {
	//signrmark := 1   // unused
	r, s, _, err := SignToRS(pri, nil, digest)
	if err != nil {
		return nil, err
	}
...
// VerifyByAsn  sign with publickey.
func VerifyByAsn(pub *PublicKey, userId []byte, src []byte, sign []byte) bool {
	var unmarshalSign Sm2Signature
	_, err := asn1.Unmarshal(sign, &unmarshalSign)
	if err != nil {
		return false
	}
	return VerifyByRS(pub, userId, src, unmarshalSign.R, unmarshalSign.S)
}
...
func CombineSignature(sign [][]byte) []byte {
	var result []byte
	for _, v := range sign {
...
func SigToPub(hash, sig, userId []byte, ee *big.Int) (*ecdsa.PublicKey, error) {
	...
	if err != nil {
		//fmt.Println("err ",err)
		return nil, errors.New("sig to pub failed:" + err.Error())
		//panic(err)
	}
	if !ok {
		//fmt.Println("ok ",ok)
		return nil, RecoverSigToPubErr
	}
